...
ifeq ($(GPU), 1) 
COMMON+= -DGPU -I/usr/local/cuda-$(CUDA_VER)/include
CFLAGS+= -DGPU
LDFLAGS+= -L/usr/local/cuda-$(CUDA_VER)/lib64 -lcuda -lcudart -lcublas -lcurand
endif

ifeq ($(CUDNN), 1) 
COMMON+= -DCUDNN  -I$(CUDNN_INSTALL_DIR)/include
CFLAGS+= -DCUDNN
LDFLAGS+= -L$(CUDNN_INSTALL_DIR)/lib64 -lcudnn
endif
